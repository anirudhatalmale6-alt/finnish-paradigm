...
def init_db():
    ...
    if not cur.fetchone():
        cur.execute('INSERT INTO users(name,email,password_hash,role,organisation,created_at) VALUES(?,?,?,?,?,?)', ('Platform Admin', ADMIN_EMAIL, hash_password(ADMIN_PASSWORD), 'admin', 'Finnish Paradigm', datetime.utcnow().isoformat()))
...
def register(payload: RegisterIn):
    ...
    try:
        cur.execute('INSERT INTO users(name,email,password_hash,role,organisation,created_at) VALUES(?,?,?,?,?,?)', (payload.name, payload.email.lower(), hash_password(payload.password), role, payload.organisation or '', datetime.utcnow().isoformat()))
...
def create_booking(payload: BookingIn):
    conn=db(); cur=conn.cursor()
    cur.execute('INSERT INTO bookings(name,email,organisation,audience,preferred_date,message,status,created_at) VALUES(?,?,?,?,?,?,?,?)',(payload.name,payload.email,payload.organisation,payload.audience,payload.preferred_date,payload.message,'new',datetime.utcnow().isoformat()))
...
def enroll(payload: EnrollmentIn, user=Depends(current_user)):
    ...
    conn=db(); cur=conn.cursor()
    try:
        cur.execute('INSERT OR IGNORE INTO enrollments(user_id,name,email,course_id,status,created_at) VALUES(?,?,?,?,?,?)',(user['id'],user['name'],user['email'],payload.course_id,'active',datetime.utcnow().isoformat()))
        conn.commit()
    finally: conn.close()
...
def mark_progress(payload: ProgressIn, user=Depends(current_user)):
    conn=db(); cur=conn.cursor()
    cur.execute('INSERT INTO progress(user_id,course_id,lesson_id,completed,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(user_id,course_id,lesson_id) DO UPDATE SET completed=excluded.completed, updated_at=excluded.updated_at', (user['id'],payload.course_id,payload.lesson_id,int(payload.completed),datetime.utcnow().isoformat()))
...
def start_assessment(payload: AssessmentStart, user=Depends(current_user)):
    conn=db(); cur=conn.cursor()
    cur.execute('INSERT INTO assessment_sessions(user_id,learner_name,course_id,ability,started_at,finished) VALUES(?,?,?,?,?,0)',(user['id'],payload.learner_name,payload.course_id,2.0,datetime.utcnow().isoformat()))
...
def answer(payload: AnswerIn, user=Depends(current_user)):
    ...
    cur.execute('INSERT INTO responses(session_id,item_id,selected,correct,created_at) VALUES(?,?,?,?,?)',(payload.session_id,payload.item_id,payload.selected,is_correct,datetime.utcnow().isoformat()))
...
def activate_paid_access(customer_email: str, product_id: str):
    ...
    cur.execute('INSERT OR IGNORE INTO enrollments(user_id,name,email,course_id,status,created_at) VALUES(?,?,?,?,?,?)', (u['id'], u['name'], u['email'], course_id, 'active', datetime.utcnow().isoformat()))
    conn.commit(); conn.close()
...
def create_checkout(payload: CheckoutIn):
    ...
    cur.execute('''INSERT INTO orders(product_id,customer_email,amount,currency,status,checkout_url,provider_ref,created_at,payment_provider,mode,metadata)
                   VALUES(?,?,?,?,?,?,?,?,?,?,?)''',(payload.product_id,payload.customer_email.lower(),amount,STRIPE_CURRENCY,'pending','', '',datetime.utcnow().isoformat(),'stripe',mode,json.dumps(meta)))
...
def record_payment_event(event: Dict[str, Any]):
    try:
        conn=db(); cur=conn.cursor()
        cur.execute('INSERT OR IGNORE INTO payment_events(event_id,event_type,provider_ref,payload,created_at) VALUES(?,?,?,?,?)', (event.get('id',''), event.get('type',''), (event.get('data') or {}).get('object',{}).get('id',''), json.dumps(event, default=str)[:20000], datetime.utcnow().isoformat()))
        conn.commit(); conn.close()
    except Exception:
        pass
...
def update_order_by_session(session, status: str):
    ...
    conn=db(); cur=conn.cursor()
    cur.execute('''UPDATE orders SET status=?, provider_ref=?, stripe_customer_id=?, stripe_payment_intent=?, paid_at=CASE WHEN ?='paid' THEN ? ELSE paid_at END WHERE id=?''', (status, session.get('id',''), session.get('customer','') or '', session.get('payment_intent','') or '', status, datetime.utcnow().isoformat(), int(order_id)))
    conn.commit(); conn.close()
    return int(order_id)
...
async def stripe_webhook(request: Request):
    ...
    if etype == 'checkout.session.completed':
        ...
        if obj.get('mode') == 'subscription' and obj.get('subscription'):
            conn=db(); cur=conn.cursor(); now=datetime.utcnow().isoformat()
        ...
        conn=db(); conn.execute('UPDATE orders SET status=?, refunded_amount=?, refunded_at=? WHERE stripe_payment_intent=?', ('refunded', refunded, datetime.utcnow().isoformat(), payment_intent)); conn.commit(); conn.close()
    elif etype in ('customer.subscription.created','customer.subscription.updated','customer.subscription.deleted'):
        status = obj.get('status','')
        current_period_end = str(obj.get('current_period_end',''))
        sub_id = obj.get('id','')
        conn=db(); conn.execute('UPDATE subscriptions SET status=?, current_period_end=?, updated_at=? WHERE stripe_subscription_id=?', (status, current_period_end, datetime.utcnow().isoformat(), sub_id)); conn.commit(); conn.close()
    elif etype == 'invoice.payment_failed':
        customer = obj.get('customer','')
        conn=db(); conn.execute('UPDATE subscriptions SET status=?, updated_at=? WHERE stripe_customer_id=?', ('past_due', datetime.utcnow().isoformat(), customer)); conn.commit(); conn.close()
    elif etype == 'invoice.paid':
        customer = obj.get('customer','')
        conn=db(); conn.execute('UPDATE subscriptions SET status=?, updated_at=? WHERE stripe_customer_id=?', ('active', datetime.utcnow().isoformat(), customer)); conn.commit(); conn.close()
...
def refund_order(payload: RefundIn, user=Depends(require_admin)):
    ...
    conn=db(); conn.execute('UPDATE orders SET status=?, refunded_amount=?, refunded_at=? WHERE id=?', ('refund_requested', payload.amount or order['amount'], datetime.utcnow().isoformat(), payload.order_id)); conn.commit(); conn.close()
...
def close_case(case_id:int, user=Depends(require_admin)):
    conn=db(); cur=conn.cursor(); cur.execute("UPDATE intervention_cases SET status='closed', updated_at=? WHERE id=?",(datetime.utcnow().isoformat(),case_id)); conn.commit(); conn.close(); return {'status':'closed'}
